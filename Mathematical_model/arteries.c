...
//      impedance_driver_(&tmstps,&Tper,&fa1ND,&fa2ND,&fa3ND,&rho,&mu_pl,&rbot,&rmin,y,&Lr,&Fr2,&q,&g,&par1,&par2,&lrr,&Z0ND,&term_ID,&Z_flag,&vaso);
...
double Tube :: Rvec (int k, int i, int j, double Q, double A, int WM)
{
  ...
  if(k==2) return(1.1*sq(Q)/A + ((j==0)?B(i,A,WM):Bh(i,A,WM)));
